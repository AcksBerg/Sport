...
export function parseSportPackage(value: unknown): SportExchangePackageV2 {
  ...
  if (![1, 2].includes(candidate.schemaVersion ?? 0) || !candidate.sport || !Array.isArray(candidate.sport.disciplines))
    throw new Error("Unbekannte oder unvollständige Sportpaketversion.");
  if (![candidate.sport.totalMaxPoints, candidate.sport.comparisonMaxPoints ?? candidate.sport.totalMaxPoints].every(Number.isFinite))
    throw new Error("Das Sportpaket enthält ungültige Zahlenwerte.");
  candidate.sport.disciplines.forEach((discipline) => {
    if (!["time", "distance", "repetitions"].includes(discipline.unit) || !Array.isArray(discipline.ageBands))
...
function cloneDiscipline(source: Discipline, retained?: Discipline): Discipline {
  ...
  discipline.ageBands = discipline.ageBands.map((band) => ({ ...band, id: bandIds.get(band.id)! }));
  discipline.formulas = discipline.formulas.map((rule) => ({ ...rule, ageBandId: bandIds.get(rule.ageBandId)!, segments: rule.segments.map((segment) => ({ ...segment, id: createId() })) }));
  discipline.tables = discipline.tables?.map((rule) => ({ ...rule, ageBandId: bandIds.get(rule.ageBandId)!, rows: rule.rows.map((row) => ({ ...row, id: createId() })) }));
  discipline.adjustmentGroups = discipline.adjustmentGroups?.map((group) => {
    const old = retained?.adjustmentGroups?.find((candidate) => normalize(candidate.label) === normalize(group.label));
    return { ...group, id: old?.id ?? createId(), options: group.options.map((option) => ({ ...option, id: old?.options.find((candidate) => normalize(candidate.label) === normalize(option.label))?.id ?? createId() })) };
  });
...
export function prepareSportReplacement(existing: Sport, imported: Sport) {
  const retainedByName = new Map(existing.disciplines.map((discipline) => [normalize(discipline.name), discipline]));
  const disciplines = imported.disciplines.map((discipline) => cloneDiscipline(discipline, retainedByName.get(normalize(discipline.name))));
  return {
